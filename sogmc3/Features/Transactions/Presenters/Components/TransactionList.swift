...
//    @State var transactionValue: String = "20.000,00"
    ...
    //Looping buat tiap transaksi yang terjadi pada tanggal tertentu
            ...
            ForEach(0..<transactionViewModel.transaction.count) { idx in
                let date: Date = transactionViewModel.transaction[idx].transactionDate
                if(Calendar.current.isDate(date, equalTo: dateNow, toGranularity: .day)){
                        HStack(alignment: .center, spacing: 8){ //HStack for detail
                            ...
                            VStack(alignment: .leading, spacing: 4){
                                Text("\(transactionViewModel.transaction[idx].transactionSubCategory)")
                                    .font(.system(size: 17))
                                Text("\(transactionViewModel.transaction[idx].transactionName)")
                                    .font(.system(size: 13))
                            }
                            .frame(maxWidth: .infinity, alignment: .leading)
                            Spacer()
                            Text("- Rp \(transactionViewModel.transaction[idx].transactionAmount)")
                        }.padding(.bottom, 17)
                    }
            }
        }
